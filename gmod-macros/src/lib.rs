...
fn check_lua_function(input: &mut ItemFn) {
    ...
    assert!(
        ...
        "There can only be one argument, and it should be a pointer to the Lua state (gmod::lua::State)"
    );
    assert!(
...
pub fn gmod13_open(_attr: TokenStream, tokens: TokenStream) -> TokenStream {
    wrap_compile_error!(tokens, {
        ...
        // No mangling
        input.attrs.push(parse_quote!(#[unsafe(no_mangle)]));
