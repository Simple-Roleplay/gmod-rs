...
use super::returns::ValuesReturned;
...
pub struct LuaSharedInterface(
    pub(crate) UnsafeCell<*mut LuaShared>,
    #[cfg(debug_assertions)] AtomicI64,
);

// SAFETY: Lua state will run on a single thread
unsafe impl Sync for LuaSharedInterface {}
...
impl LuaSharedInterface {
    ...
    pub(crate) fn debug_assertions(&self) {
        assert!(
            ...
            "The Lua state has not been initialized yet. Add `#[gmod::gmod13_open]` to your module's gmod13_open function to fix this. You can also manually set the Lua state with `gmod::set_lua_state(*mut c_void)`"
        );

        let thread_id = u64::from(std::thread::current().id().as_u64()) as i64;
        match self.1.compare_exchange(
            -1,
            thread_id,
            ...
            Ok(_) => unreachable!(),
            Err(remembered_thread_id) => assert_eq!(
                thread_id, remembered_thread_id,
                "Tried to access the Lua state from another thread! The Lua state is NOT thread-safe, and should only be accessed from the main thread."
            ),
        }
    }
...
pub static LUA_SHARED: LuaSharedInterface = LuaSharedInterface(
...
pub struct LuaShared {
    ...
    pub lual_checkinteger:
        Symbol<'static, unsafe extern "C-unwind" fn(state: LuaState, arg: i32) -> LuaInt>,
    ...
    pub lua_tointeger:
        Symbol<'static, unsafe extern "C-unwind" fn(state: LuaState, index: i32) -> LuaInt>,
    pub lua_tonumber:
