...
impl LuaState {
    ...
    pub unsafe fn get_type(&self, index: i32) -> &str {
        unsafe {
            ...
            std::str::from_utf8_unchecked(lua_type_str.to_bytes())
        }
    }
    ...
    pub unsafe fn get_field(&self, index: i32, k: LuaString) {
        unsafe { (LUA_SHARED.lua_getfield)(*self, index, k) }
    }
    ...
    pub unsafe fn pcall(&self, nargs: i32, nresults: i32, errfunc: i32) -> i32 {
        unsafe { (LUA_SHARED.lua_pcall)(*self, nargs, nresults, errfunc) }
    }
    ...
    pub unsafe fn pcall_ignore(&self, nargs: i32, nresults: i32) -> bool {
        match unsafe { self.pcall(nargs, nresults, 0) } {
            LUA_OK => true,
            LUA_ERRRUN => {
                unsafe {
    ...
    pub unsafe fn load_string(&self, src: LuaString) -> Result<(), LuaError> {
        let lua_error_code = unsafe { (LUA_SHARED.lual_loadstring)(*self, src) };
        if lua_error_code == 0 {
            Ok(())
        } else {
    ...
    pub unsafe fn load_buffer(&self, buff: &[u8], name: LuaString) -> Result<(), LuaError> {
        let lua_error_code = unsafe {
            (LUA_SHARED.lual_loadbuffer)(*self, buff.as_ptr() as LuaString, buff.len(), name)
        };
        if lua_error_code == 0 {
            Ok(())
        } else {
    ...
    pub unsafe fn load_file(&self, path: LuaString) -> Result<(), LuaError> {
        let lua_error_code = unsafe { (LUA_SHARED.lual_loadfile)(*self, path) };
        if lua_error_code == 0 {
            Ok(())
        } else {
            Err(LuaError::from_lua_state(*self, lua_error_code))
    ...
    pub unsafe fn push_globals(&self) {
        unsafe { (LUA_SHARED.lua_pushvalue)(*self, LUA_GLOBALSINDEX) }
    }

    #[inline(always)]
    pub unsafe fn push_registry(&self) {
        unsafe { (LUA_SHARED.lua_pushvalue)(*self, LUA_REGISTRYINDEX) }
    }
    ...
    pub unsafe fn set_field(&self, index: i32, k: LuaString) {
        unsafe { (LUA_SHARED.lua_setfield)(*self, index, k) }
    }

    #[inline(always)]
    pub unsafe fn get_global(&self, name: LuaString) {
        unsafe { (LUA_SHARED.lua_getfield)(*self, LUA_GLOBALSINDEX, name) }
    }

    #[inline(always)]
    pub unsafe fn set_global(&self, name: LuaString) {
        unsafe { (LUA_SHARED.lua_setfield)(*self, LUA_GLOBALSINDEX, name) }
    }
    ...
    pub unsafe fn call(&self, nargs: i32, nresults: i32) {
        unsafe { (LUA_SHARED.lua_call)(*self, nargs, nresults) }
    }
    ...
    pub unsafe fn get_metatable(&self, index: i32) -> i32 {
        unsafe { (LUA_SHARED.lua_getmetatable)(*self, index) }
    }

    #[inline(always)]
    pub unsafe fn check_table(&self, arg: i32) {
        unsafe { (LUA_SHARED.lual_checktype)(*self, arg, LUA_TTABLE) }
    }

    #[inline(always)]
    pub unsafe fn check_function(&self, arg: i32) {
        unsafe { (LUA_SHARED.lual_checktype)(*self, arg, LUA_TFUNCTION) }
    }

    #[inline(always)]
    pub unsafe fn check_integer(&self, arg: i32) -> LuaInt {
        unsafe { (LUA_SHARED.lual_checkinteger)(*self, arg) }
    }

    #[inline(always)]
    pub unsafe fn check_number(&self, arg: i32) -> f64 {
        unsafe { (LUA_SHARED.lual_checknumber)(*self, arg) }
    }
    ...
    pub unsafe fn new_metatable(&self, name: LuaString) -> bool {
        unsafe { (LUA_SHARED.lual_newmetatable)(*self, name) == 0 }
    }
    ...
    pub unsafe fn debug_getinfo_from_ar(
        &self,
        ar: &mut LuaDebug,
        what: LuaString,
    ) -> Result<(), ()> {
    ...
    pub unsafe fn debug_getinfo_at(&self, level: i32, what: LuaString) -> Option<LuaDebug> {
        unsafe {
            let mut ar = MaybeUninit::uninit();
            if (LUA_SHARED.lua_getstack)(*self, level, ar.as_mut_ptr()) != 0
                && (LUA_SHARED.lua_getinfo)(*self, what, ar.as_mut_ptr()) != 0
            {
